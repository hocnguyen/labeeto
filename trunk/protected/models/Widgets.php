...
class Widgets extends CActiveRecord
{
    ...
    public static function show($alias, $return = false){
        ...
        if( !$model ){
            $lang   = 'en';
            $model  = self::model()->findByAttributes(array('alias' => trim($alias), 'language' => $lang)); 
        }
        //parse page link
        ...
        //Case info-footer then remove title
        if( $model->title == 'infor footer' || $model->alias == 'infor-footer' )
            $html = $model->content;
        else if ($model->title){
