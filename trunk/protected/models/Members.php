...
{	
    ...
    public function rules()
    {
        ...
        return array(
			...
			array('sourcefrom, data, birthday, last_logged,state,shipping_state,shipping_address,shipping_email', 'safe'),
    ...
    public function relations()
    {
        ...
        return array(
            ...
            'auctionJoins' => array(self::HAS_MANY, 'AuctionJoins', 'user_id'),
            'auctionViews' => array(self::HAS_MANY, 'AuctionViews', 'user_id'),
			'auctionVotes' => array(self::HAS_MANY, 'AuctionVotes', 'user_id'),
			'bids' => array(self::HAS_MANY, 'Bids', 'bidder_id'),
...
ON bidstransactions.id = mem.id";
    ...
    public function checkLoginUser( $username, $password, $campaign_id ){
        ...
        $sql = "SELECT b.*, campaign.template_id FROM ( SELECT members.*, affiliate.id AS aff_id,affiliate.url, affiliate.title, affiliate.description, affiliate.shop_name, affiliate.business_type_id FROM members
                ...
                INNER JOIN campaign
                ON b.aff_id = campaign.affiliate_id
    ...
    function getActivityAuctionsbyMem( $user_id = 0, $is_end = 0 ){
        $sql = "SELECT * FROM(
                    ...
                    SELECT auction_id,bidder_id,price,created,SUM(sumofbid) AS total,is_win,product_id FROM(
                    ...
                    HAVING b.bidder_id = '$user_id' ) AS lista
                    LEFT JOIN products
    ...
    function getUsernameShop(){
        $tablemember = $this->tableName();
        return Yii::app()->db->createCommand()
                                        ->select('id,username')
                                        ->from($tablemember)
                                        ->queryAll();
