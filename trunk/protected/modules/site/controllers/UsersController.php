...
class UsersController extends SiteBaseController {
	...
	public function actionverify()
	{
		...
		if($email != '')
		{
			if($member = Members::model()->findByAttributes(array('email'=>$email)))
			{
				...
				{
					$code = '';
					$verierror = Yii::t('error','Invalid code');
				}
			}
			else
			{
				$email == '';
				$code = '';
				$verierror = Yii::t('error','Invalid email or code');
			}
		}
			...
			if( $model->validate() )
			{
				...
				if($identity->authenticate() )
				{
                    ...
                    elseif($check_role_login == 1){
                        ...
                        Yii::app()->user->setFlash('success', Yii::t('global', 'Please check your email to active account before login !'));
                        $this->redirect('/login');
                    }
    ...
    {
        if(Yii::app()->user->role == 'affiliate') $this->redirect('/affiliate');
        $model = new LoginForm;
        ...
        if( isset($_POST['LoginForm']) )
        {
            ...
            if( $model->validate() )
            {
                ...
                // Redirect
                if(Yii::app()->user->role == 'affiliate') { // check affiliate by webservice after
